...
dpg.create_context()
dpg.create_viewport(title="Rhythm Player",large_icon="icon.ico",small_icon="icon.ico")
pygame.mixer.init()
global state
state=None
...
def play(sender, app_data, user_data):
	...
	if user_data:
		...
		if pygame.mixer.music.get_busy():
			...
			dpg.configure_item("csong",default_value=f"Now Playing : {ntpath.basename(user_data.musica)}")
			...
			with dpg.child_window(autosize_x=True,height=80,no_scrollbar=True):
				with dpg.group(horizontal=True):
					with dpg.group(horizontal=True):
						...
						dpg.add_button(label="Next",tag="next",show=True,callback=next_,width=65,height=30)
